...
Computes pair (D, N) where the total area is N*sqrt(D) and prints explicit
quadrilateral geometry (a,b,c,d,e,f) for small (D,N).
"""
...
# Limits for printing outputs (Summary & explicit examples)
MAX_D_SUMMARY = 150 # Maximum discriminant D to display in the summary
MAX_D_PRINT = 10    # Maximum discriminant D to display detailed examples for
MAX_N_PRINT = 10    # Maximum rational factor N to display detailed examples for
...
def texQ(x):
    """Helper to format rationals nicely for LaTeX."""
    if x.denominator() == 1:
        return str(x)
...
for (e, D) in sorted(Triangles.keys()):
    ...
    for i in range(m):
        ...
        for j in range(i, m):
            ...
            # Keep the first (minimal complexity) quadrilateral found for pair (D, N)
            if key not in Examples:
                Examples[key] = (a, b, c, d, e, f, q1, q2, q_total)

# ==============================================================================
# SUMMARY & EXPLICIT EXAMPLES FOR SMALL (D, N)
# ==============================================================================
byD = {}
for D, N in sorted(Examples.keys()):
    if D < MAX_D_SUMMARY:  # Filter discriminants for summary
        byD.setdefault(D, []).append(N)

print("=" * 75)
print(f"Summary of found N values per discriminant D (showing D < {MAX_D_SUMMARY}):")
print("=" * 75)
for D in sorted(byD.keys()):
    print(f"D = {D:3d} | N values: {byD[D][:10]}")
print("=" * 75)

print("\n" + "=" * 75)
print(f"EXPLICIT QUADRILATERALS FOR SMALL PAIRS (D <= {MAX_D_PRINT}, N <= {MAX_N_PRINT}):")
print("=" * 75)

found_any = False
for (D, N) in sorted(Examples.keys()):
    if D <= MAX_D_PRINT and N <= MAX_N_PRINT:
        found_any = True
        a, b, c, d, e, f, q1, q2, q = Examples[(D, N)]
        
        print(f"\n[ Discriminant D = {D}, Factor N = {N} ]")
        print(f"  Sides (a,b,c,d) : ({a}, {b}, {c}, {d})")
        print(f"  Diagonals (e,f) : ({e}, {f})")
        print(f"  Triangle areas  : {q1}*sqrt({D}) + {q2}*sqrt({D}) = {q}*sqrt({D})")
        print("  LaTeX format    :")
        print(f"  {N}\\sqrt{{{D}}} & \\left({texQ(a)},{texQ(b)},{texQ(c)},{texQ(d)},{texQ(e)},{texQ(f)}\\right) & \\left({texQ(q1)}\\sqrt{{{D}}},{texQ(q2)}\\sqrt{{{D}}}\\right) \\\\")

if not found_any:
    print(f"No explicit examples found with D <= {MAX_D_PRINT} and N <= {MAX_N_PRINT}.")
    print("Try increasing MAX_NUM or MAX_DEN search bounds.")
